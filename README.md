...
- **Payroll Circuit (Planned):** In the future, a dedicated circuit will batch-process shielded transfers to multiple employees simultaneously, proving that the sum of the encrypted output notes equals the input note without leaking individual salaries.
...
*   **Private Payroll:** The Payroll Employer Console UI is beautifully built and fully functional, allowing an employer to select employees, set amounts, and process payments in a single click. However, true ZK-shielding of these specific transfers on-chain is a future milestone. For the sake of the hackathon demo, the payroll execution currently relies on standard Soroban token transfers under the hood rather than utilizing the fully shielded ZK note pool.
