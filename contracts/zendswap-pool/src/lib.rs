...
impl ZendSwapPool {
    ...
    pub fn set_rate(
        ...
        if !(5_000_000..=20_000_000).contains(&new_rate) {
            return Err(Error::InvalidRate);
        }
...
mod tests {
    ...
    fn test_set_rate_bounds_fail() {
        ...
        assert!(client
            .try_set_rate(&admin, &0u64, &1u64, &9_200_000, &10_000_001)
            .is_err());
        assert!(client
            .try_set_rate(&admin, &0u64, &1u64, &9_200_000, &9_999_999)
            .is_err());
        assert!(client
            .try_set_rate(&admin, &0u64, &1u64, &4_999_999, &10_000_000)
            .is_err());
        assert!(client
            .try_set_rate(&admin, &0u64, &1u64, &20_000_001, &10_000_000)
            .is_err());
        assert!(client
