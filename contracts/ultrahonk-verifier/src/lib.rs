...
fn validate_proof_g1_points(env: &Env, proof: &Bytes) -> bool {
    let validate_at_offset = |offset: u32| -> bool {
        ...
        proof.slice(offset..offset + 128).copy_into_slice(&mut chunk);
...
mod test {
    ...
    fn test_verification_success() {
        ...
        // 1. merkle_root: 0x1efa4f36e981c724252a85046380f1f1f396be7801006394a67dda888a148d03
        let merkle_root_bytes: [u8; 32] = [
            0x1e, 0xfa, 0x4f, 0x36, 0xe9, 0x81, 0xc7, 0x24,
            0x25, 0x2a, 0x85, 0x04, 0x63, 0x80, 0xf1, 0xf1,
            0xf3, 0x96, 0xbe, 0x78, 0x01, 0x00, 0x63, 0x94,
            0xa6, 0x7d, 0xda, 0x88, 0x8a, 0x14, 0x8d, 0x03,
        ];
        public_inputs.push_back(BytesN::from_array(&env, &merkle_root_bytes));

        // 2. nullifier_hash: 0x24560659afb92134b13df0b6c7f98395c271fcc35a3c84c76b812931c8ac2ea5
        let nullifier_hash_bytes: [u8; 32] = [
            0x24, 0x56, 0x06, 0x59, 0xaf, 0xb9, 0x21, 0x34,
            0xb1, 0x3d, 0xf0, 0xb6, 0xc7, 0xf9, 0x83, 0x95,
            0xc2, 0x71, 0xfc, 0xc3, 0x5a, 0x3c, 0x84, 0xc7,
            0x6b, 0x81, 0x29, 0x31, 0xc8, 0xac, 0x2e, 0xa5,
        ];
